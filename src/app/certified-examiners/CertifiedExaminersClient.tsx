...
interface Props {
  initialExaminers: ExaminerProfile[];
}

export default function CertifiedExaminersClient({ initialExaminers }: Props) {
  const adminPanelRef = useRef<ExaminerAdminPanelHandle | null>(null);
  const [examiners, setExaminers] = useState<ExaminerProfile[]>(initialExaminers);
  ...
  // 서버에서 받은 초기 데이터가 없을 때만 클라이언트에서 페칭
  useEffect(() => {
  ...
  // 셔플을 한 번만 수행하도록 최적화
  const randomizedExaminers = useMemo(() => {
    if (examiners.length === 0) return [];
    return shuffleArray(examiners);
  }, [examiners]);
        ...
        <div className="certified-examiners__section-container">
          ...
          {isLoading ? (
            <div className="certified-examiners__directory-grid">
              {/* 스켈레톤 로더 추가 */}
              {[...Array(9)].map((_, index) => (
                <div key={`skeleton-${index}`} className="certified-examiners-card animate-pulse">
                  <div className="certified-examiners-card__media">
                    <div className="certified-examiners-card__image certified-examiners-card__image--placeholder bg-gray-200">
          ...
          {!isLoading && visibleExaminers.length > 0 && (
          <div className="certified-examiners__directory-grid">
